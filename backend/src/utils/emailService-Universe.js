...
exports.sendHospitalRegistrationNotification = async (hospitalData) => {
  try {
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@healthinsura360.com';
    ...
    const mailOptions = {
      ...
      to: adminEmail,
      subject: `🏥 New Hospital Registration Request - ${hospitalData.name}`,
      html: `
        <h2>New Hospital Registration Request</h2>
        <p>A new hospital has registered and is pending approval:</p>
        <ul>
          <li><strong>Name:</strong> ${hospitalData.name}</li>
          <li><strong>Email:</strong> ${hospitalData.email}</li>
          <li><strong>Phone:</strong> ${hospitalData.phone}</li>
          <li><strong>Contact Person:</strong> ${hospitalData.contactPerson}</li>
          <li><strong>Location:</strong> ${hospitalData.city}, ${hospitalData.state}</li>
        </ul>
        <p><a href="${process.env.FRONTEND_URL}/admin/hospitals/pending">Click here to review</a></p>
      `
    };
