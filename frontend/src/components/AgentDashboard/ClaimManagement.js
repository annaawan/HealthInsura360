...
function ClaimManagement({ agent }) {
  ...
  const viewClaimDetails = (claim) => {
    ...
    console.log('Documents:', claim.documents);
  ...
  const handleRetryPayment = async (claim) => {
    if (!window.confirm(`Retry payment of ${formatCurrency(claim.approved_amount)} for claim #${claim.claim_id}?`)) {
        return;
    }
    ...
    try {
        ...
        alert(`❌ Payment failed: ${error.response?.data?.message || 'Failed to process payment'}`);
    } finally {
        setActionLoading(false);
    }
  };

  // Approve claim with type
  const handleApproveClaimWithType = async (coverageType) => {
    ...
    if (coverageType === 'partial' && approvedAmt > currentClaim.remaining_coverage) {
        alert(`Cannot approve ${formatCurrency(approvedAmt)}. Remaining coverage is only ${formatCurrency(currentClaim.remaining_coverage)}.`);
        return;
    }
        ...
        Warning: Claim amount exceeds remaining coverage. Only partial approval possible.
              ...
              <div className="mb-6">
                <h4 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                  ...
                  Supporting Documents ({selectedClaim.documents?.length || 0})
                </h4>
          ...
          <div className="bg-white rounded-xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            {currentClaim.remaining_coverage <= 0 && (
              <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <p className="text-sm text-red-600 mb-3">
                  This policy has {formatCurrency(currentClaim.remaining_coverage)} remaining coverage.
                </p>
                <button
                  onClick={() => {
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (type === 'full_coverage') {
                        ...
                        message = `✅ Insurance pays ${formatCurrency(approvedAmount)}`;
                        ...
                        message = `💰 Deductible applied. Insurance pays ${formatCurrency(approvedAmount)}`;
                        ...
                        message = `📊 ${copay}% co-pay applied. Insurance pays ${formatCurrency(approvedAmount)}`;
                      } else if (type === 'partial') {
                        showManualInput = true;
                        approvedAmount = Math.min(remainingCoverage, claimAmount);
                        message = `✏️ Enter custom amount. Max: ${formatCurrency(remainingCoverage)}`;
                      } else if (type === 'excess_coverage') {
                        if (claimAmount > remainingCoverage) {
                          approvedAmount = remainingCoverage;
                          message = `⚠️ Insurance pays full coverage ${formatCurrency(approvedAmount)}`;
                        } else {
                          approvedAmount = Math.min(claimAmount, remainingCoverage);
                          message = `✅ Insurance pays ${formatCurrency(approvedAmount)}`;
                        }
                      ...
                      if (messageDiv) {
                        messageDiv.innerHTML = `<div class="p-2 rounded bg-green-50 text-green-700">${message}</div>`;
                      }
                    }}
                  >
                    ...
                    {currentClaim.deductible_amount > 0 && (
                      <option value="with_deductible">💰 Apply Deductible</option>
                    )}
                    {currentClaim.co_pay_percentage > 0 && (
                      <option value="with_copay">📊 Apply Co-pay</option>
                    )}
                    <option value="partial">✏️ Partial Coverage</option>
                    {currentClaim.claim_amount > currentClaim.remaining_coverage && currentClaim.remaining_coverage > 0 && (
                      <option value="excess_coverage">⚠️ Excess Coverage</option>
                    )}
                  </select>
                </div>
